...
#define TAG "com.bea.paypal"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Modulopaypal_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Modulopaypal.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ModulopaypalBindings::lookupGeneratedInit(
...
static void Modulopaypal_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Modulopaypal_getBinding)->GetFunction());
}

static void Modulopaypal_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ModulopaypalBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ModulopaypalBinding = {
	"com.bea.paypal",
	Modulopaypal_init,
	Modulopaypal_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_bea_paypal_ModulopaypalBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.bea.paypal", &ModulopaypalBinding);
	titanium::KrollBindings::addExternalLookup(&(::ModulopaypalBindings::lookupGeneratedInit));
}
